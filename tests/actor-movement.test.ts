...
vi.mock('../apps/client/src/game/unitAnimation', () => ({
  createUnitSprite: vi.fn(), setUnitAnimation: vi.fn(), updateUnitBody: vi.fn(), facingFromDelta: () => 'east',
}));
...
function viewAt(actor: WorldActor): ActorView {
  ...
  return { container, sprite: { anims: { pause() {}, resume() {}, timeScale: 1 } },
    motion: createPlannedMotion(point, MOVEMENT_TICK_MS), facing: 'south',
    hasPath: false, walkTimeScale: 1, selected: false, dead: false } as unknown as ActorView;
}
...
describe('actor rendering across bushes', () => {
  it('keeps mobs at their normal visible and animation pace while heroes slow down', () => {
    ...
    expect(mobView.sprite.anims.timeScale).toBe(heroView.sprite.anims.timeScale * 2);
