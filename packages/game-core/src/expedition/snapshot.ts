...
import { array, record, same, string } from '../snapshot-values';
...
export function serializeExpedition(state: ExpeditionState, content: GameContent): string {
  return JSON.stringify({ version: 1, difficultyId: state.difficultyId ?? 'normal', contentHash: hashValue(content), world: serializeExploration(state.world), clearedPoiIds: state.clearedPoiIds, activePoiId: state.activePoiId, combat: state.combat ? serializeSnapshot(state.combat) : null, completed: state.completed, ...(state.failed !== undefined ? { failed: state.failed } : {}), ...(state.roaming ? { roaming: state.roaming } : {}) });
}
...
export function deserializeExpedition(json: string, content: GameContent): ExpeditionState {
  ...
  const value = record(JSON.parse(json), 'expedition', ['version', 'difficultyId', 'contentHash', 'world', 'clearedPoiIds', 'activePoiId', 'combat', 'completed', 'failed', 'roaming']);
  ...
  if (roaming) {
    ...
    if (combat && (activePoiId !== roaming.active!.mobId || combat.encounterId !== 'roaming'
      ...
      || combat.seed !== roamingBattleSeed(world.graph.seed, world.currentChunkId, roaming.battleSerial)
    ...
    if (!poi || clearedPoiIds.includes(poi.id)
      ...
      || world.actors.some(actor => actor.path.length)
      || combat.seed !== encounterSeed(world.graph.seed, poi.id)
  ...
  return { version: 1, difficultyId, world, clearedPoiIds, activePoiId, combat, completed: value.completed, ...(value.failed !== undefined ? { failed: value.failed } : {}), ...(roaming ? { roaming } : {}) };
}
