...
export function equipmentFromLoadout(loadout: HeroLoadout, content: GameContent): StarterEquipment[] {
  return loadout.equipment.map(({ itemId, slot }) => {
    const item = resolveEquipmentItem(content.equipmentCatalog?.items, itemId);
...
export function equippedHero(hero: UnitDefinition, loadout: HeroLoadout, content: GameContent): UnitDefinition {
  if (!hero.anatomy || !content.equipmentCatalog) return applyNativeSkillRarities(hero, loadout, content);
  ...
  return applyNativeSkillRarities({ ...hero, stats, skillIds: [...hero.skillIds.filter(id => !content.skills.find(skill => skill.id === id)?.tags.includes('LEARNABLE')), ...loadout.skills.filter((id): id is string => !!id)],
    anatomy: { ...hero.anatomy, equipment, ...(setBonuses.length ? { setBonuses } : {}) } }, loadout, content);
}
...
export function contentWithLoadouts(content: GameContent, loadouts?: Record<string, HeroLoadout>): GameContent {
  ...
  const key = JSON.stringify(Object.keys(loadouts).sort().map(id => [id, loadouts[id].equipment, loadouts[id].skills, loadouts[id].nativeSkillRarities]));
...
export function battleLoadouts(state: CoopState, ids: readonly string[]): Record<string, HeroLoadout> | undefined {
  ...
  return Object.fromEntries(ids.map(id => {
    const hero = state.progression!.heroes[id];
    return [id, { equipment: hero.equipment, skills: hero.skills, ...(hero.nativeSkillRarities ? { nativeSkillRarities: hero.nativeSkillRarities } : {}) }];
  }));
}
...
function storeRemovedFinds(old: HeroProgress, next: HeroProgress, removed: readonly RemovedFind[], content: GameContent, reservedIds: readonly string[] = [], ownerId = 'preview'): HeroProgress {
  ...
  for (const item of removed) {
    const definition = item.kind === 'equipment' ? resolveEquipmentItem(content.equipmentCatalog?.items, item.definitionId)
      : content.skills.find(skill => skill.id === item.definitionId && skill.tags.includes('LEARNABLE'));
...
function resolveRewardChoice(old: HeroProgress, choice: RewardResolution, content: GameContent, reservedIds: readonly string[], ownerId: string): HeroProgress {
  ...
  for (const selection of choice.skills) {
    ...
    if (reward.kind !== 'skill' || !content.skills.some(skill => skill.id === reward.definitionId && skill.tags.includes('LEARNABLE'))) throw new Error('Это не изучаемый навык.');
  ...
  if (new Set(learned.map(baseSkillId)).size !== learned.length) throw new Error('Один навык нельзя экипировать в оба слота.');
...
export function previewInventoryEquip(old: HeroProgress, inventoryId: string, target: InventoryTarget, content: GameContent, body?: HeroBody, reservedIds: readonly string[] = [], ownerId?: string): HeroProgress {
  ...
  if (target === 'skill0' || target === 'skill1') {
    if (reward.kind !== 'skill' || !content.skills.some(skill => skill.id === reward.definitionId && skill.tags.includes('LEARNABLE'))) throw new Error('Это не изучаемый навык.');
    if (old.skills.some(id => id && baseSkillId(id) === baseSkillId(reward.definitionId))) throw new Error('Этот навык уже экипирован.');
    skills = [...old.skills];
    ...
    const item = resolveEquipmentItem(content.equipmentCatalog.items, reward.definitionId);
...
const sameEquipment = (left: HeroLoadout['equipment'], right: HeroLoadout['equipment']): boolean => left.length === right.length
  && left.every(item => right.some(other => other.slot === item.slot && other.itemId === item.itemId));

/** Convert directly from the committed body to one final outfit, never through intermediate swaps. */
function bodyForLoadout(body: HeroBody, definition: UnitDefinition): HeroBody {
  const fresh = startHeroBody(definition), next = { ...body };
  for (const part of BODY_PARTS) {
    const resource = body[part], max = fresh[part].max;
    next[part] = resource.max === max ? resource : { max, lost: resource.lost ?? false, current: resource.lost ? bodyPartLossThreshold(max)
      : Math.max(bodyPartLossThreshold(max) + 1, Math.min(max, Math.floor(resource.current / resource.max * max))) };
  }
  return next;
}

/** Resolve owned instances first, then validate and replace the complete outfit atomically. */
function automaticEquipment(state: CoopState, actorId: string, hero: HeroProgress, content: GameContent): { hero: HeroProgress; consumed: AdventureReward[] } {
  const actor = state.actors.find(entry => entry.id === actorId);
  const definition = content.characters.find(entry => entry.id === actorId);
  if (!hero.autoEquipment || !actor || !definition || !content.equipmentCatalog || state.failed || state.completed
    || getCoopBattle(state, actorId) || actor.body && !isBodyAlive(actor.body)) return { hero, consumed: [] };
  const body = actor.body && bodyForLoadout(actor.body, equippedHero(definition, hero, content));
  const plan = optimizeEquipment(definition, hero, content, body, state.characterIds.length);
  if (!plan.changed) return { hero, consumed: [] };
  const inventory = hero.inventory ?? [];
  const retained = new Set<string>(), selectedIds = new Set<string>();
  const candidates: { id: string; itemId: string }[] = [];
  const selections = plan.equipment.map(selection => {
    let token: string, itemId: string;
    if ('inventoryId' in selection) {
      const reward = inventory.find(entry => entry.id === selection.inventoryId);
      if (!reward || reward.kind !== 'equipment') throw new Error('Выбранного предмета больше нет в сумке.');
      token = `inventory:${reward.id}`; itemId = reward.definitionId;
      selectedIds.add(reward.id);
    } else {
      const worn = hero.equipment.find(entry => entry.slot === selection.equippedSlot && entry.itemId === selection.itemId);
      if (!worn) throw new Error('Надетое снаряжение уже изменилось.');
      token = `equipped:${worn.slot}`; itemId = worn.itemId;
      retained.add(worn.slot);
    }
    const item = resolveEquipmentItem(content.equipmentCatalog!.items, itemId);
    if (!item || !equipmentItemFitsSlot(item, selection.slot)) throw new Error('Предмет не подходит для выбранного слота.');
    if (body && ('inventoryId' in selection || selection.equippedSlot !== selection.slot)) {
      const parts = equipmentBodyPartsForSlot(item, selection.slot);
      const missing = item.weapon ? parts.some(part => body[part].lost) : parts.length > 0 && parts.every(part => body[part].lost);
      if (missing) throw new Error('Для выбранного предмета не хватает действующих частей тела.');
    }
    candidates.push({ id: token, itemId });
    return { rewardId: token, slot: selection.slot };
  });
  const chosen = confirmEquipmentChoice([], candidates, selections, content.equipmentCatalog.items);
  const equipment = chosen.equipment.map(item => ({ itemId: item.id!, slot: item.slot }));
  // Equal outfits should not churn identical bag copies or create historical IDs.
  if (sameEquipment(equipment, hero.equipment)) return { hero, consumed: [] };
  const consumed = inventory.filter(entry => selectedIds.has(entry.id));
  const removed = removedEquipment(equipmentFromLoadout(hero, content).filter(item => !retained.has(item.slot)));
  const reserved = [...state.removedRewardIds, ...Object.values(state.progression?.heroes ?? {})
    .flatMap(entry => [...entry.rewards, ...(entry.inventory ?? [])].map(reward => reward.id))];
  const next = storeRemovedFinds(hero, { ...hero, equipment, inventory: inventory.filter(entry => !selectedIds.has(entry.id)) },
    removed, content, reserved, actorId);
  return { hero: next, consumed };
}

/** Commit one final loadout; only explicit inventory changes request automatic selection. */
export function applyHeroLoadout(state: CoopState, actorId: string, hero: HeroProgress, content: GameContent,
  options: { optimizeEquipment?: boolean } = {}): CoopState {
  const progression = state.progression ?? initialProgress(content, state.characterIds);
  const old = progression.heroes[actorId], definition = content.characters.find(entry => entry.id === actorId);
  if (!old || !definition) throw new Error('Герой не найден.');
  const automatic = options.optimizeEquipment ? automaticEquipment(state, actorId, hero, content) : { hero, consumed: [] };
  hero = automatic.hero;
  if (hero.equipment !== old.equipment && sameEquipment(hero.equipment, old.equipment)) hero = { ...hero, equipment: old.equipment };
  const equipmentChanged = hero.equipment !== old.equipment;
  if (equipmentChanged && content.equipmentCatalog) confirmEquipmentChoice(equipmentFromLoadout(hero, content), [], [], content.equipmentCatalog.items);
  const equipped = equipmentChanged ? equippedHero(definition, hero, content) : undefined;
  const actors = equipmentChanged ? state.actors.map(actor => {
    if (actor.id !== actorId || !actor.body) return actor;
    return { ...actor, body: bodyForLoadout(actor.body, equipped!) };
  }) : state.actors;
  const removedRewardIds = automatic.consumed.length ? [...new Set([...state.removedRewardIds,
    ...automatic.consumed.flatMap(reward => [reward.id, coopRewardId(`${actorId}:${reward.kind}`, reward.definitionId)])])] : state.removedRewardIds;
  return { ...state, progression: { ...progression, heroes: { ...progression.heroes, [actorId]: hero } }, actors, removedRewardIds };
}

export function changeLoadout(state: CoopState, actorId: string, command: Extract<CoopCommand, { type: 'equip' | 'learn' | 'discard-reward' | 'resolve-rewards' | 'collect-reward' | 'collect-rewards' | 'equip-inventory' | 'set-auto-equipment' }>, content: GameContent): CoopState {
  const progression = state.progression ?? initialProgress(content, state.characterIds);
  ...
  const removed: RemovedFind[] = [];
  let taken: string[];
  if (command.type === 'set-auto-equipment') {
    if (typeof command.enabled !== 'boolean') throw new Error('Некорректный режим подбора снаряжения.');
    if (!!old.autoEquipment === command.enabled) return state;
    return applyHeroLoadout(state, actorId, { ...old, autoEquipment: command.enabled }, content, { optimizeEquipment: command.enabled });
  } else if (command.type === 'collect-rewards') {
    const ids = command.rewardIds;
    if (!Array.isArray(ids) || ids.length < 1
      || ids.some(id => typeof id !== 'string' || !id.length || id.length > 1024)
      || new Set(ids).size !== ids.length) throw new Error('Укажите неповторяющийся список наград для сбора.');
    const available = new Map(old.rewards.map(reward => [reward.id, reward]));
    const collected = new Set([...inventory.map(reward => reward.id), ...state.removedRewardIds]);
    // Validate the complete selection before moving anything. Retries remain
    // harmless even if some finds have since been equipped or discarded.
    for (const id of ids) if (!available.has(id) && !collected.has(id)) throw new Error('Одна из наград недоступна для сбора.');
    const selected = new Set(ids);
    const rewards = old.rewards.filter(reward => selected.has(reward.id) && !collected.has(reward.id));
    if (!rewards.length) return state;
    hero.inventory = [...inventory, ...rewards.map(reward => ({ ...reward }))];
    taken = rewards.map(reward => reward.id);
  } else if (command.type === 'collect-reward') {
    ...
    if (command.slot !== 'skill0' && command.slot !== 'skill1') hero.autoEquipment = false;
    ...
    if (command.equipment.length) hero.autoEquipment = false;
  ...
  const next = applyHeroLoadout(state, actorId, hero, content,
    { optimizeEquipment: command.type === 'collect-reward' || command.type === 'collect-rewards' });
  return { ...next, removedRewardIds: [...new Set([...next.removedRewardIds, ...taken,
...
export function restoreProgress(value: unknown, content: GameContent, ids: readonly string[], tick: number): AdventureProgress {
  ...
  for (const id of ids) {
    ...
    hero.inventory ??= [];
    if (hero.autoEquipment !== undefined && typeof hero.autoEquipment !== 'boolean') throw new Error('Invalid automatic equipment preference');
    if (hero.npcPurchases !== undefined && (!Array.isArray(hero.npcPurchases) || hero.npcPurchases.some(value => typeof value !== 'string' || !value.length || value.length > 1024)
      || new Set(hero.npcPurchases).size !== hero.npcPurchases.length)) throw new Error('Invalid NPC purchases');
    if (hero.nativeSkillRarities !== undefined && (!hero.nativeSkillRarities || typeof hero.nativeSkillRarities !== 'object'
      || Object.keys(hero.nativeSkillRarities).length !== 3 || !(['class', 'active', 'passive'] as const).every(slot => REWARD_RARITIES.includes(hero.nativeSkillRarities![slot])))) throw new Error('Invalid native ability rarities');
    for (const skill of hero.skills) if (skill !== null && !content.skills.some(item => item.id === skill && item.tags.includes('LEARNABLE'))) throw new Error('Unknown learned skill');
    const learned = hero.skills.filter(skill => skill !== null);
    if (new Set(learned.map(baseSkillId)).size !== learned.length) throw new Error('Duplicate learned skill');
    ...
    for (const reward of [...hero.rewards, ...hero.inventory]) {
      if (!reward || typeof reward.id !== 'string' || !reward.id || rewardIds.has(reward.id) || (reward.kind === 'equipment' ? !resolveEquipmentItem(content.equipmentCatalog?.items, reward.definitionId)
        : reward.kind !== 'skill' || !content.skills.some(skill => skill.id === reward.definitionId && skill.tags.includes('LEARNABLE')))) throw new Error('Unknown saved reward');
