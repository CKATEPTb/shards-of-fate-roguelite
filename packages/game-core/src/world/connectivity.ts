import type { ChunkExit, MapValidation, WorldChunk, WorldGraph, WorldNode } from '@shards/shared';
...
interface ChunkSummary {
  ...
  count: number;
  exits: GateSummary[];
  points: Array<{ id: string; kind: string; region: number }>;
}
...
export function analyzeWorldConnectivity(graph: WorldGraph, overrides?: ReadonlyMap<string, WorldChunk>): WorldConnectivityAnalysis {
  ...
  if (altars.length !== 1 || altars[0] !== graph.altarNodeId) errors.push('Missing or misplaced winter altar');
  if (errors.length) return result;
