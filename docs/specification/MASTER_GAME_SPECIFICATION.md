...
Через 20 минут исследования на любой сложности призывается
...
Алтарь вызывает босса рядом с нажавшим героем. Полный отсчёт следующего
сезона начинается после победы над текущим боссом. Пока в комнате идёт
хотя бы один бой, общий таймер босса стоит на паузе, включая кооператив.
В бою на его месте показан таймер выбора действия: 30 секунд после
анимации начала хода. По истечении срока ход игрока пропускается с обычным
срабатыванием эффектов конца хода. У каждого отдельного боя свой срок;
ходы противников выполняются автоматически. Победа над всеми четырьмя
боссами завершает поход.
...
# 15. Equipment model: скрытая сумка

В Run сохраняются текущая экипировка персонажа и скрытая сумка. Отдельного
окна сумки, сетки хранения, ограничения вместимости и интерфейса торговли нет.
Нажатие на находку переносит её в сумку без изменения экипировки.

Через слот персонажа игрок выбирает совместимый предмет или изученный навык,
сравнивает его с текущим и подтверждает замену. Все снятые вещи и заменённые
изученные навыки возвращаются в сумку того же героя. Двуручное оружие возвращает
обе снятые вещи из рук, а само при снятии возвращается одним предметом.
Два кольца учитываются независимо. Остальные находки в сумке сохраняются.

Закрытие добычи с несобранными находками требует предупреждения и удаляет
только оставленные награды. Текущие правила и совместимость сохранений:
[ADR 0027](../adr/0027-loot-pickup-and-slot-inventory.md).
...
`HeroProgress` хранит `equipment`, `skills`, `inventory` и ещё не собранные `rewards`.
...
## 14.3 Reward flow со скрытой сумкой

``` text
ENCOUNTER WON
   ↓
HOST GENERATES PERSONAL REWARDS
   ↓
PLAYER COLLECTS A REWARD INTO HIDDEN INVENTORY
   ↓
PLAYER OPENS A CHARACTER SLOT
   ↓
PLAYER SEES CURRENT-vs-CANDIDATE COMPARISON
   ↓
EQUIP / DECLINE
   ↓
ALL DISPLACED ITEMS RETURN TO THE SAME HERO'S INVENTORY
   ↓
OTHER INVENTORY ENTRIES REMAIN AVAILABLE
```

Изучаемый навык работает тем же образом: новый навык заменяет один из двух
найденных навыков после подтверждения, а предыдущий возвращается в сумку.

------------------------------------------------------------------------

## 14.4 Атомарная замена экипировки
...
validate slot / ownership / reward result
      ↓
remove NEW ITEM from the hero's inventory
      ↓
equip NEW ITEM
      ↓
store EVERY displaced OLD ITEM in the hero's inventory
      ↓
broadcast the compact loadout command
```

Возвращённый предмет остаётся личным запасом владельца и выбирается через
соответствующий слот персонажа. Замена не создаёт предметы на земле и не
...
состоянии: `EQUIPPED`, `INVENTORY`, `REWARD_PENDING` или `DISCARDED`.
Сумка сохраняется между участками и при продолжении похода. Возвращённые
находки получают уникальные ID без повторного использования уже потраченных
ID команд, чтобы повторная доставка не создавала копии.
