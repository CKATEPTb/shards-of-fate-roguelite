...
Ориентиры: Act I 25 мин, Act II 22, Act III 20; configurable. Warnings
5:00/1:00/0:30/0:10. На нуле обычный encounter завершается, затем
принудительный Boss; auto-defeat нет. В каждом акте ровно один reachable
Boss Altar в Winter, не у первого входа. Активация→party vote. Ранний
вызов→configurable extra loot/rarity/special chest/currency.
...
1.  выбрать предмет как награду;
2.  если предмет выигран и подходит по слоту --- сразу экипировать его
    вместо текущего;
3.  отказаться/не выбирать предмет.

Снятый при замене предмет **не перемещается в сумку**. Он физически
остаётся в игровом мире в той же точке/POI, где игрок подобрал и
экипировал новый предмет. Таким образом замена работает как обмен: новый
предмет надевается, старый предмет выкладывается на землю на место
полученного предмета и становится доступным для последующего подбора,
пока группа находится в соответствующем состоянии/чанке. Поэтому не
...
├── ring2
├── mainHand
├── offHand?       // только если archetype поддерживает
├── scroll1
└── scroll2
```
