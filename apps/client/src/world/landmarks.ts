...
export function drawLandmarks(scene: Phaser.Scene, chunk: WorldChunk, cleared: Set<string>, roaming = false): Phaser.GameObjects.Container {
  ...
  for (const poi of chunk.pois) {
    ...
    if (poi.kind === 'campfire') {
      art.fillStyle(0xffb769, 0.09).fillCircle(x, y, 36);
      ...
      art.fillStyle(0xa14d2e).fillRect(x - 5, y + 2, 11, 3);
      art.fillStyle(0xe99b4f).fillRect(x - 3, y + 3, 3, 1).fillRect(x + 3, y + 2, 2, 1);
