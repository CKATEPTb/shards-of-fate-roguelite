...
export function BossSummonDialog({ season, onCancel, onConfirm }: { season: Season; onCancel: () => void; onConfirm: () => void }) {
  ...
  return <GameDialog title="Призыв босса" onClose={onCancel} className="boss-summon-dialog">
    ...
    <p className="boss-summon-next">{nextSeason ? `До появления босса ${bossSeasonNames[nextSeason]} начнётся новый отсчёт: 20 минут.` : 'Последний сезон. Победите всех четырёх боссов, чтобы завершить поход.'}</p>
