...
export function BossSummonDialog({ season, onCancel, onConfirm }: { season: Season; onCancel: () => void; onConfirm: () => void }) {
  ...
  return <GameDialog title="Призыв босса" onClose={onCancel} className="boss-summon-dialog">
    ...
    <p className="boss-summon-description">Он появится рядом с вами вместе с двумя сопровождающими и всей стаей нападёт на героев поблизости.</p>
    <p className="boss-summon-next">{nextSeason ? `Только после победы над всей стаей начнётся отсчёт 20 минут до появления босса ${bossSeasonNames[nextSeason]}.` : 'Последний сезон. Победите всех четырёх боссов и их стаи, чтобы завершить поход.'}</p>
